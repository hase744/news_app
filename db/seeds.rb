...
technology = [
   ...
   ビッグデータ 仮想化 ブロックチェーン コーディング デジタル ハイパーテキスト API GUI UI UX クリプトグラフィ IOWN
    ...
    {sort:"bio",words:%w(
        細胞 不老不死 老化 IPS細胞 ES細胞 ゲノム編集 クローニング DNA合成 RNA編集 タンパク質合成 遺伝子発現 プラスミド
         クリスパー-Cas9 PCR プロテオミクス ゲノム解析 バイオインフォマティクス タンパク質異常 シーケンシング 細胞培養
          バイオセンサー シークエンシング ゲノムマッピング 分子クローニング 分子生物学 ゲノムシーケンシング 遺伝子療法
           転写因子 分子診断 ゲノム研究 クロマトグラフィー 生物学的工学 細胞工学 シクロトロン ゲノムプロジェクト
            ナノバイオテクノロジー ゲノム改変 分子診療 遺伝子変異解析
            )},
    {sort:"energy",words:%w(核融合 全個体 全樹脂 燃料電池)},
    {sort:"vehicle",words:%w(ドローン 空飛ぶ 超電導 無人 自動運転 電動)},
    #{sort:"company",words:%w(テスラ アップル Apple マイクロソフト Microsoft グーグル Google メタ社 Meta TSMC CATL NTT NEC Sony OpenAI)},
    {sort:"person",words:%w(イーロン ザッカーバーグ ティムクック サム・アルトマン)},
]
...
politics = [
    ...
    {sort:"party",words:%w(自民党 民主党 公明党 日本維新 共産党 れいわ新選組 賛成党 政治家女子48党 総裁 幹事長 総務会長 政務調査会長 特別補佐)}
]

life = [
    {sort:"general",words:%w(グッズ 便利 小物 生活 対策グッズ 用品 コスパ 掘り出し物 ベストバイ アイテム 防寒 用品) },
    {sort:"item",words:%w(文具 文房具 手帳 ノート ボールペン 財布 靴 ホルダー 家電)},
    {sort:"phrase",words:%w(買うべき 買ってよかった 買って良かった 買わないと損 これ１台 これ１冊 これ１本 おすすめ オススメ 一推し イチオシ 激推し)},
    {sort:"brand",words:%w(無印 ニトリ ダイソー DAISO キャンドゥ セリア Seria ワークマン 百均 100均 100円 )},
]

lifehack = [
    {sort:"method",words:%w(活用法 活用術 生活術 裏技 収納法 ミニマル ミニマリスト 機能 用途 方法 使い方 習慣 コツ)},
    {sort:"phrase",words:%w(買うべき 買ってよかった 買って良かった 買わないと損 これ１台 これ１冊 おすすめ 激推し ラク 時短)},
    {sort:"brand",words:%w(無印 ニトリ ダイソー DAISO Seria セリア キャンドゥー ワークマン IKEA イケア 百均 100均 100円)},
    life.first
]

beauty = [
    {sort:"cosmetic",words:%w(化粧 美容 コスメ メイク ファンデーション リップスティック アイシャドウ マスカラ ブラッシュ アイライナー 
    ...
    {sort:"body",words:%w(肌 スキン 皮膚 しわ シワ しみ シミ たるみ 鼻 毛穴 角栓 皮脂 まつ毛 まつげ ニキビ にきび ほうれい線)},
    {sort:"shape",words:%w(整形 2重 ツルツル すべすべ つや ツヤ 美白 癖毛 剛毛 軟毛 さらさら サラサラ 赤み 墨 乾燥)},
...
business = [
    {sort:"sales",words:%w(営業 セールス 商談 売る 売れる 客 クロージング アポ プレゼン)},
    {sort:"accounting",words:%w(収益 コスト 利益 ROI 資産 負債 利益 コスト キャッシュフロー マージン 単価)},
    {sort:"management",words:%w(経営 マネジメント チームビルディング リーダーシップ  組織文化 ガバナンス オペレーション 戦略計画 目標設定)},
    {sort:"marketing",words:%w(
...
channel_seeds = [
    ...
    {"youtube_id"=>"", "url"=>"https://www.youtube.com/@suntv_news_Japan", "categories" => %w(politics)},
